...
class LAGCN(nn.Module):
    def __init__(self, concat, nfeat, nhid, nclass, dropout, use_mlp=False):
        ...
        if use_mlp:
            self.mlp = MLPLayer(concat*nhid, nclass)
        else:
            self.gc2 = GraphConvolution(concat*nhid, nclass)

    def forward(self, x_list, adj, final_adj=None):
        hidden_list = []
